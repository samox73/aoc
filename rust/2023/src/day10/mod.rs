...
fn count_inside(mut grid: Grid) -> usize {
    let mut count = 0;
...
impl Grid {
    fn get_mut(&mut self, (x, y): (isize, isize)) -> &mut Pipe {
        self.pipes
            .get_mut(y as usize)
            .unwrap()
            .get_mut(x as usize)
    ...
    fn get_start(&mut self) -> &mut Pipe {
        self.get_mut((self.start.1, self.start.0))
    }
}
...
fn find_loop_length(mut grid: Grid) -> (u32, Grid) {
    let mut count = 0;
